...
export function WaitingState() {
  const { status, pendingReconnect, reconnectAttempts, toggleMatchWatching, checkGameProcess } = useGameStore();
  const { t } = useI18n();

  const isPaused = status === "PAUSED";
  const isWaitingForGame = status === "WAITING_FOR_GAME" && !pendingReconnect;
  const isLoading = status === "CONNECTING" || status === "RECONNECTING" || status === "IDLE" || pendingReconnect;
  ...
  return (
    <div className="flex flex-col flex-1 min-h-0 relative overflow-hidden">
      ...
      <div className="flex-1 flex items-center justify-center px-4 min-h-0 relative z-10">
        <div key={activeView} className="flex flex-col items-center animate-smooth-appear w-full max-w-[280px]">
          ...
          {isPaused ? (
             ...
             <div className="flex flex-col items-center text-center w-full">
                 ...
                 <h2 className="text-[14px] font-black text-accent-gold tracking-widest mb-1.5">
                    {pendingReconnect || status === "RECONNECTING" ? t("waiting.reconnecting") : t("header.connecting")}
                 </h2>
                 {status === "RECONNECTING" && (
